...
export function QueuePanel({ events, intervalMs = 1_000, now = Date.now, ttlMs = 5_000 }: QueuePanelProps) {
  ...
  useEffect(() => {
    ...
    setRows((current) => {
      ...
      return Array.from(next.values()).sort((a, b) => a.identifier.localeCompare(b.identifier))
    })
  }, [events, now])
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      setRows((current) => current.filter((row) => currentTime - row.lastSeen <= ttlMs))
    }, intervalMs)

    return () => window.clearInterval(timer)
  }, [intervalMs, now, ttlMs])

  if (rows.length === 0) {
    return (
      <section aria-label="Queue" style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
        No blocked issues
      </section>
    )
  }
    ...
    <section aria-label="Queue">
      <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', listStyle: 'none', margin: 0, padding: 0 }}>
        {rows.map((row) => (
          ...
          >
            <strong>{row.identifier}</strong> <span style={{ color: 'var(--text-secondary)' }}>blocked by</span>{' '}
            {row.blockers.join(', ')}
          </li>
        ))}
      </ul>
