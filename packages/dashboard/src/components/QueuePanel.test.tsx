...
function queueEvent(overrides: Partial<QueueEventPayload> = {}): QueueEventPayload {
  return {
    issue_id: 'issue-1',
    identifier: 'ZII-50',
    blockers: 'ZII-49',
...
describe('QueuePanel', () => {
  ...
  it('renders multiple blockers', () => {
    render(<QueuePanel events={[queueEvent({ identifier: 'ZII-52', blockers: 'ZII-49,ZII-50' })]} />)
  ...
  it('evicts entries after six seconds', async () => {
    let nowMs = 0
    render(<QueuePanel events={[queueEvent()]} intervalMs={1} now={() => nowMs} />)
    expectInDocument(screen.getByText('ZII-50'))
  ...
  it('refresh resets eviction', async () => {
    ...
    const { rerender } = render(<QueuePanel events={[first]} intervalMs={1} now={() => nowMs} />)
    expectInDocument(screen.getByText('ZII-50'))

    nowMs = 4_000
    await act(async () => {
      rerender(<QueuePanel events={[first, queueEvent({ blockers: 'ZII-49,ZII-51' })]} intervalMs={1} now={() => nowMs} />)
    })
